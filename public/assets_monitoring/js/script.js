document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateTanggalWaktu() {
    ...
    const now = new Date();
    const teksTanggal = `${hari[now.getDay()]}, ${now.getDate()} ${bulan[now.getMonth()]} ${now.getFullYear()} ${now.getHours().toString().padStart(2, "0")}:${now.getMinutes().toString().padStart(2, "0")}`;
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      document.getElementById("suhu").innerText = Math.round(data.main.temp);
      document.getElementById("cuaca-kondisi").innerText = data.weather[0].description;
      document.getElementById("kelembapan").innerText = `${data.main.humidity}%`;
      document.getElementById("angin").innerText = `${data.wind.speed} km/jam`;
      document.getElementById("arah").innerText = arah;
      document.getElementById("icon-cuaca").src =
