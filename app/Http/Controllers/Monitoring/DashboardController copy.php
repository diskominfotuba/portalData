...
class DashboardController extends BaseController
{
    public function index()
    {
        ...
        // Penduduk data
        $urlPenduduk = "https://webapi.bps.go.id/v1/api/list/model/data/lang/ind/domain/1808/var/1233/th/123/key/9c5801d21f56653dacd1bcd2e806b4df";
        // Parawisata data
        $urlParawisata = 'https://webapi.bps.go.id/v1/api/list/model/data/lang/ind/domain/1808/var/1313/th/124/key/9c5801d21f56653dacd1bcd2e806b4df';
        ...
        $responsePenduduk = Http::get($urlPenduduk);
        $responseParawisata = Http::get($urlParawisata);
        ...
        if ($responsePenduduk->ok()) {
            $jsonPenduduk = $responsePenduduk->json();
            $datacontentPenduduk = $jsonPenduduk['datacontent'] ?? [];
            $jumlah_penduduk = (int) ($datacontentPenduduk['591233631230'] ?? 0);
        }

        // Proses data Parawisata
        if ($responseParawisata->ok()) {
            $jsonParawisata = $responseParawisata->json();
            $datacontentParawisata = $jsonParawisata['datacontent'] ?? [];
            $jumlah_parawisata = (int) ($datacontentParawisata['9131301240'] ?? 0);
        }
